...
contract KeyHolder is ERC725 {
    ...
    function addKeybyCorporation(bytes32 _key, uint256 _purpose, uint256 _type)
        public
        returns (bool success)
    {
        require(keys[_key].key != _key, "Key already exists"); // Key should not already exist
        ...
        emit KeyAddedbyCorporation(_key, _purpose, _type);

        return true;
    }
    
     function addKeybyManager(bytes32 _key, uint256 _purpose, uint256 _type)
        public
        returns (bool success)
    {
        require(keys[_key].key != _key, "Key already exists"); // Key should not already exist
        ...
        emit KeyAddedbyManager(_key, _purpose, _type);
    ...
    function removeKeybyCorporation(bytes32 _key)
        public
        returns (bool success)
    {
        require(keys[_key].key == _key, "No such key");
        require(keys[_key].purpose >= 3);
        emit KeyRemovedbyCorporation(keys[_key].key, keys[_key].purpose, keys[_key].keyType);
    ...
    function removeKeybyManager(bytes32 _key)
        public
        returns (bool success)
    {
        require(keys[_key].key == _key, "No such key");
        require(keys[_key].purpose >= 4);
        emit KeyRemovedbyManager(keys[_key].key, keys[_key].purpose, keys[_key].keyType);
